...
export const createReservation = (fullName, date, time, numPersons) => async (dispatch, getState) => {
    ...
    try {
      ...
      const { data } = await http.post(`/api/reservations`, {fullName, date, time, numPersons}, {
        headers: {
          Authorization: `Bearer ${userInfo.token}`,
        },
      });
...
export const payOrder = (order, paymentResult) => async (dispatch, getState) => {
  ...
  try {
    const { data } = await http.put(`/api/orders/${order._id}/pay`,paymentResult, {
      headers: { Authorization: `Bearer ${userInfo.token}` },
    });
...
export const listOrderPersonal = () => async (dispatch, getState) => {
  ...
  try {
    const { data } = await http.get(`/api/orders/personal`, {
      headers: {
        Authorization: `Bearer ${userInfo.token}`,
      },
    });
...
export const deleteReservation = (orderId) => async (dispatch, getState) => {
  ...
  try {
    const { data } = await http.delete(`/api/reservations/${orderId}`, {
      headers: {
        Authorization: `Bearer ${userInfo.token}`,
      },
    });
...
export const deliverOrder = (orderId) => async (dispatch, getState) => {
  ...
  try {
    const { data } = await http.put(`/api/orders/${orderId}/deliver`,{}, {
      headers: { Authorization: `Bearer ${userInfo.token}` },
    });
