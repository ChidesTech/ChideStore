...
export const listProducts = ({pageNumber ='', name = '', category=''}) => async (dispatch) =>{
   ...
   try{
    const {data} = await http.get(`/api/products?pageNumber=${pageNumber}&name=${name}&category=${category}`);
   dispatch({
   ...
   try {
       const {data} =await http.get(`/api/products/${prodId}`);
   ...
   export const updateProduct =(product)=> async(dispatch, getState)=>{
    ...
    const {userSignin:{userInfo}} = getState();
    try {
        const {data} = await http.put(`/api/products/${product._id}`,product,{
            headers: {
              Authorization: `Bearer ${userInfo.token}`,
            },
        })
   ...
   export const deleteProduct =(productId)=> async(dispatch, getState)=>{
    ...
    const {userSignin:{userInfo}} = getState();
    try {
        const {data} = await http.delete(`/api/products/${productId}`,{
            headers: {
              Authorization: `Bearer ${userInfo.token}`,
            },
        })
   ...
   export const listCategories = () => async (dispatch) =>{
    ...
    export const createCategory =(category)=> async(dispatch, getState)=>{
        ...
        const {userSignin:{userInfo}} = getState();
        try {
            const {data} = await http.post(`/api/categories`,category,{
                headers: {
                  Authorization: `Bearer ${userInfo.token}`,
                },
            })
       ...
       export const detailsCategory = (categoryId) =>async(dispatch)=>{
        ...
        export const updateCategory =(category)=> async(dispatch, getState)=>{
            ...
            const {userSignin:{userInfo}} = getState();
            try {
                const {data} = await http.put(`/api/categories/${category._id}`,category,{
                    headers: {
                      Authorization: `Bearer ${userInfo.token}`,
                    },
                })
           ...
           export const listProductCategories = () => async (dispatch) =>{
            ...
            export const createReview =(prodId, review)=> async(dispatch, getState)=>{
                ...
                const {userSignin:{userInfo}} = getState();
                try {
                    const {data} = await http.post(`/api/products/${prodId}/reviews`,review,{
                        headers: {
                          Authorization: `Bearer ${userInfo.token}`,
                        },
                    })
