...
<select  style={{padding:"1rem", borderRadius:"15px", border:"2px solid steelblue",fontWeight:"bolder", outline:"steelblue"}} 
...
{categories.length>0 ? categories.map(category=>(<option style={{padding:"1rem", fontSize:"1.6rem"}}  value={category._id}>{category.name}</option>)
 ...
 {/* <input type="text" id="category" required placeholder="Enter Product Category" value={category} onChange={(e)=> setCategory(e.target.value)}/> */}
          ...
          <div style={{display:"flex",justifyContent:"space-between" ,flexDirection:"row-reverse", marginTop:"1rem"}} >
              ...
              <Link to="/product-list" style={{width:"40%", backgroundColor:"transparent",textAlign: "center",  borderColor:"transparent", color:"steelblue", boxShadow:"1px 1px 1px 1px steelblue"}} type="button" className="btn">Cancel</Link>
